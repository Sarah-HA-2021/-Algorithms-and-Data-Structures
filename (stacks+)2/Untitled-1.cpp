...
int sum_q (vector <int> &st,int v,int L,int R,int l,int r)
{
    if ((l<=L) and (R<=r))
    return st[v];
    ...
    int mid = (L + R) / 2;
    return min(sum_q(st,2*v+1, L, mid, l, r) ,sum_q(st,2*v+2, mid+1, R, l, r));
...
int modefiy_q (vector <int> &st,int v,int L,int R,int i,int value)
{
    if (L==R)
    return st[v]=value;
